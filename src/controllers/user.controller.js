    ...
    const registerUser = asyncHandler( async(req,res)=>{
        ...
        const loginUser = asyncHandler(async(req,res)=>{
            ...
            const {accessToken,refreshToken}= await generateAcessTokenAndRefreshToken(user._id);  //for generating acess and refresh token , it will used later
        ...
        const logoutUser = asyncHandler(async(req,res)=>{
            await User.findByIdAndUpdate(
                req.user._id,   // req.user se dudha
                {
                    $unset:{
        ...
        const refreshAccessToken= asyncHandler(async(req,res)=>{   // REFRESH TOKEN SE YHA ACCESS AND REFRESH TOKEN BNARE H
        ...
        const updateAccountDetails = asyncHandler(async(req,res)=>{
            ...
            if(!(username|| email)){
                throw new ApiError(404,"Can't Change username and password")
            }
        ...
        const updateUserAvatarImage = asyncHandler(async(req,res)=>{
            try {
                ...
                if(user.avatar){
                    const avatarId = user.avatar.match(/upload\/(?:[^/]+\/)?(.+?)(?=\.[^/.]+$|$)/)?.[1];
                    // const avatarId = user.avatar.split("/").pop().split(".")[0]     // url ko "/" iske perpective m seprate krenge then pop last value return krega then split by "." then split se jo value ayegi uske [0] index ko save kr lenge
                    try {
                        await cloudinary.uploader.destroy(avatarId)
                    } catch (error) {
                        throw new ApiError(400,error.message||"Error in deleting old avatar")
                    }
                }
                const avatar = await uploadOnCloudinary(avatarLocalPath)
                if(!avatar.url){
                ...
                const updateAvatar = await User.findByIdAndUpdate(req.user._id,         // no need to save bcz this method findoneandupdate do save also
                    {
                        $set:{
                            avatar:avatar.url
                        }
                ...
                res.status(200)
                .json(
                    new ApiResponse(200,updateAvatar,"Avatar image saved sucessfully")
                )
            } catch (error) {
        ...
        try {
            ...
            if(user.coverImage){
            const coverId = user.coverImage.match(/upload\/(?:[^/]+\/)?(.+?)(?=\.[^/.]+$|$)/)?.[1];  // regex for cover id
                // const coverId= user.coverId.split("/").pop().split(".")[0]
                try {
                    await cloudinary.uploader.destroy(coverId)
                } catch (error) {
                    throw new ApiError(400,error.message||"Error while deleting coverImage")
                }
            }
            const updateCoverImg = await User.findByIdAndUpdate(req.user._id,
                {
                    $set:{
            ...
            res.status(200).
            json(
                new ApiResponse(200,updateCoverImg,"Cover Image uploaded sucessfully")
            )
            } 
            catch (error) {
                        ...
                        pipeline:[
                            {
                                $lookup:{
                                    ...
                                    pipeline:[        // that's why we use pipeline here
                                        {
                                            $project:{ // project is used to what we want to project(show)
                                                username:1,
                                                fullName:1,
                                                avatar:1,
                                            }
                            ...
                            },// ${lookup} always returns an array, even if there's only 1 match
                            ...
                            {     // yha tk data nikal gya owner ka and next pipeline for sending neat data to frontend
                                $addFields:{       // addfield add extra value to that pipeline array
                                    owner:{
                                        $first: "$owner" // now it will send 1st data(object)
                                                        //${addFields} with ${first} converts the array to a single object
                                    }
                                }
                            }
                        ]
